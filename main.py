...
def show_alternatives(window, category, search_range):
...
def menu_callback(window):
    ...
    alt_button = ttk.Button(window, text="Show alternatives", command = lambda : show_alternatives(window, list(window.scale_dicts.keys()).index(window.menu_holder.get()) + 4, rs1.getValues()))
    alt_button.grid(row=7, padx=20, pady=10, columnspan=3)

    # calculation button
    # + 4 bias
    calc_button = ttk.Button(window, text='Calculate', command = lambda : calculate(window, window.hotels))
    calc_button.grid(row=9, padx=20, pady=10, columnspan=3)
    return
